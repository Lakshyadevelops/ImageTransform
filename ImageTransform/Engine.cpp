...
bool Engine::build(std::string onnxModelPath, const std::array<float, 3>& subVals, const std::array<float, 3>& divVals,
    ...
    m_logger.CustomLog("Searching for engine file with name: " + m_engineName);

    if (doesFileExist(m_engineName)) {
        m_logger.CustomLog("Engine found, not regenerating...");
        return true;
    }
    ...
    m_logger.CustomLog("Engine not found, generating. This could take a while...");
    ...
    if (input0Batch == -1) {
        m_logger.CustomLog("Model supports dynamic batch size");
    }
    else if (input0Batch == 1) {
        m_logger.CustomLog("Model only supports fixed batch size of 1");
